...
\]
The optimization is quickly solved using an alternative direction method of multipliers (ADMM) algorithm [@boyd2011distributed]. Furthermore, using $m> N^{\frac{2}{3}}$ (with regular spacing), is enough for statistical optimality of the solution $\hat{\theta}$ [@ortiz2018mbs]. 
...
The $\tt{plot}$ function can also be used to see fits at other values of $\lambda$, such as lambda corresponding to 1 standard error rule (for greater parsimony) or an arbitrary number:
...
For multivariate data, the total variation problem becomes difficult. Let $(x_1,\dlots,x_p)$ denote $p$ covariates. Furthermore, we define a multi-index $r=(r_1,\ldots,r_p)$, where $r_j\in \{0,1\}$ and $|r| = \sum_{j=1}^p r_j$. In our approach, we approximate the penalty
\[
P(f) = \sum_{r\in \{0,1\}^p} \int \left|\frac{\partial^{|r|}}{\partial^{r_1}x_1 \ldots \partial^{r_p}x_p}f(x)\right|\partial x_1\ldots \partial x_p,
\]
where $\{0,1\}^p\equiv \{0,1\} \times \ldots \times \{0,1\}$, i.e. the Cartesian product. In the bivariate setting, our penalty parameter becomes
\[
...
\]
When $P(f)$ is finite, solutions have bounded total variation. $\bf{MultivarTV}$ continues to approximate $P(f)$ using differences across a mesh. For details, see @ortiz2018mbs. We can fit the multivariate problem using the same ADMM algorithm as in the univariate case, so $\tt{mvtv}$ uses the same algorithm regardless of dimensionality. 
...
As we increase the sample size to $N=5,000$, the total variation solution improves (Figure 7). The thin plate spline solution takes a large amount of time to calculate, so we do not present it.

```{r figure7, echo=TRUE, fig.cap="Figure 7. Total variation solution for noisy towers with many data points.", fig.width=4, fig.height=4}
set.seed(117)
z1 <- runif(5000)
z2 <- runif(5000)
z3 <- towers(z1,z2)
...
```{r figure8, echo=TRUE, fig.cap= "Figure 7. Residuals plot for perfectly noisy data.", fig.width=3.5}
set.seed(117)
n <- 300
p <- 3
m <- 4
...
For uniform noisy data regressed on uniform noisy trivariate data, we expect fitted values at the mean and evenly dispersed residuals, which is what we see with some small wiggling (Figure 8). 
